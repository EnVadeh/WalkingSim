...
void main(){
	...
	outColor = texture(scatteringDensityLUT, vec3(uv, 0.2)).xyz; //+ texture(rayleighLUT, vec3(uv, 0.2)).xyz; //+ texture(mieLUT, vec3(uv, 0.2)).xyz + texture(transmittanceLUT, vec2(uv)).xyz;
}